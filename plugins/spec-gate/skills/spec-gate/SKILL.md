...
- `docs/backlog/NN-nome.md`: spec de cada PBI (item de backlog); é o contrato de comportamento observável a partir do qual os testes black-box são derivados. `SPEC.md` não existe mais no fluxo atual.
...
- `.specgate/gate.json`: os gates mecânicos de PO (backlog, testes, aceite, ambiguidade), cada um com `checkpoint`, `pbi`, `rodada`, `status` e `opened_at_seq`. Sempre reescrito por inteiro (Write do array completo), nunca por Edit ou Bash. Um gate decidido é registro imutável; nunca tente reabri-lo reescrevendo por cima.
