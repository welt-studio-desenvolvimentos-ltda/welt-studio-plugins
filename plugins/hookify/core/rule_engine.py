...
# Tool name → event type mapping (shared by hook entry points)
TOOL_EVENT_MAP = {
    'Bash': 'bash',
    'Edit': 'file', 'Write': 'file', 'MultiEdit': 'file', 'Update': 'file',
    'Read': 'read', 'Glob': 'read', 'Grep': 'read', 'LS': 'read',
}


def is_blocking_result(result: dict, hook_event: str) -> bool:
    """Check if result is a blocking decision (needs stderr + exit 2)."""
    if not result:
        return False
    ...
    def evaluate_rules(self, rules: List[Rule], input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if blocking_rules:
            ...
            if hook_event == 'Stop':
                ...
                # For other events (UserPromptSubmit, etc.)
                return {
                    "decision": "block",
                    "systemMessage": combined_message
                }
